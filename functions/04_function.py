...
def create_mask_ti(ti_name):
    '''
    Create a mask based on the ti.
    The ti must be stored relatively to this file to /data/TI/pickle/.
    ti_name : pickle file.
    '''
    
    with open('datas/TI/pickle/'+ti_name,'rb') as file:
        ti = pickle.load(file)
...
def create_mask_zone(ti_name, position):
    '''
    Create a mask based on the extracted zone.
    The ti must be stored relatively to this file to /data/TI/pickle/.
    ti_name : pickle file.
    position : position of the extracted zone.
    '''
...
def deeSse_run_ti_pyr(ti_Img, mask_ti, hd_pts, n=12, t=0.05, f=0.50, nReal=1):
    '''
    DeeSse run, where the simulation use the TI as conditionning data.
...
def deeSse_run_zone_pyr(ti_Img, mask_zone, hd_pts, n=12, t=0.05, f=0.50, nReal=1):
    '''
    DeeSse run, where the simulation use the TI as conditionning data.
    Simulation with pyramides.
    The MPS simulation complete the missing part of the TI.
