...
data_DEM.raster[data_DEM.raster.data==np.min(data_DEM.raster.data)] = np.nan
mask_where_nodata = ~np.isnan(data_DEM.raster.data)
...
test_trend = str(input('Do you want to remove the trend of the TI ? (True or False) : '))

if test_trend == 'True':
    
    #TI to points data set like
    ti  = np.copy(data_DEM.raster.data)
    
    #Create the cell coordinnates
    x     = np.arange(0,ti.shape[1],1)
    y     = np.arange(0,ti.shape[0],1)
    X,Y   = np.meshgrid(x,y)
    
    #Remove the nan data
    ti_f = np.copy(ti.flatten())
    X    = X.flatten()
    X    = X[~np.isnan(ti_f)]
    Y    = Y.flatten()
    Y    = Y[~np.isnan(ti_f)]
    Z    = ti_f[~np.isnan(ti_f)]
    ...
    ti_trend = np.copy(ti)
    ti_trend[~np.isnan(ti_trend)] = Z_poly
    new_ti = np.copy(data_DEM.raster.data)
    new_ti[~np.isnan(ti_trend)] = new_ti[~np.isnan(ti_trend)]-ti_trend[~np.isnan(ti_trend)]
    data_DEM.raster[:,:] = new_ti
...
for lineit in range(num_img):
    ...
    while res == False:
        ...
        New_TI = np.copy(data_DEM.raster.data)
        New_TI[rdm_Y:(rdm_Y+int(size_img_c[1])),rdm_X:int((rdm_X+size_img_c[0]))] = np.nan
    ...
    if test_trend !='True':
        trend_cut = np.copy(raster_cut)
        trend_cut[:,:] = 0

    else:
        trend_cut  = ti_trend[rdm_Y:(rdm_Y+int(size_img_c[1])),rdm_X:int((rdm_X+size_img_c[0]))]
    ...
    mask_box_ti[~np.isnan(mask_box_ti)]  = 1    
    mask_box_ti[np.isnan(mask_box_ti)]   = 0
